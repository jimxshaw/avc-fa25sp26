...
- ESP32 V2V bridge to command the UGV

Mission behavior:
1. Connect to the drone through MAVLink.
2. Connect to the ESP32 V2V bridge.
3. Open the ZED camera and read camera intrinsics from the SDK.
4. Detect ArUCo markers, giving priority to small marker ID 6 before ID 5.
5. Estimate marker pose relative to the camera.
6. Convert that pose into body-frame forward/right/down errors.
7. Switch the drone to GUIDED, arm, and take off.
8. Once takeoff is confirmed, command the UGV to move forward 15 yards.
9. After takeoff, follow the target for 10 seconds before beginning precision landing.
10. After the follow phase completes, begin precision descent only when centered enough.
11. Switch to LAND when low and tightly centered.

Important:
- This script sends the UGV a forward-distance command using the existing
  "GOTO:<distance_m>,0" format supported by your uploaded ground_station file.
- Your current ground_station(3).py sets SPEED_MPH = 0.8, so if you truly want
  the UGV at 3.0 mph, change SPEED_MPH on the UGV side too.
"""
...
# ESP32 bridge connection from UAV side
ESP32_BRIDGE_PORT = "/dev/ttyUSB0"
ESP32_BRIDGE_BAUD = 115200
...
# UGV motion request
UGV_TARGET_SPEED_MPH = 3.0       # requested mission speed (UGV side must match)
UGV_MOVE_DISTANCE_YD = 15.0
YD_TO_M = 0.9144
UGV_MOVE_DISTANCE_M = UGV_MOVE_DISTANCE_YD * YD_TO_M
...
def handle_sigint(sig, frame):
    global running
    running = False
    log_event("[INFO] Stopping script...")
...
def disable_ugv_avoidance(master):
    """
    Disable ArduPilot avoidance so the UAV does not try to dodge the UGV.
    """
...
def connect_bridge():
    log_event(f"[INFO] Connecting to V2V bridge: {ESP32_BRIDGE_PORT}")
    bridge = v2v_bridge.V2VBridge(ESP32_BRIDGE_PORT, baud=ESP32_BRIDGE_BAUD, name="UAV-Bridge")
    bridge.connect()
    time.sleep(0.5)
    return bridge


def command_ugv_move_forward(bridge):
    """
    Use the existing GOTO parser in ground_station(3).py:
      GOTO:<x_m>,<y_m>
    and y must be 0 for body-forward motion.
    """
    goto_msg = f"GOTO:{UGV_MOVE_DISTANCE_M:.3f},0"
    log_event(
        f"[INFO] Commanding UGV to move {UGV_MOVE_DISTANCE_YD:.1f} yd "
        f"({UGV_MOVE_DISTANCE_M:.3f} m). Requested speed is {UGV_TARGET_SPEED_MPH:.1f} mph "
        f"but actual speed is controlled by the UGV ground station."
    )

    # Send multiple times for reliability across the bridge
    for _ in range(3):
        bridge.send_message(goto_msg)
        time.sleep(0.2)
...
def main():
    ...
    try:
        ...
        # Once the UAV has taken off, tell the UGV to move
        command_ugv_move_forward(bridge)
        ...
        while running:
            ...
            if found:
                ...
                # Start 10-second follow timer when ID 5 is first acquired
                if follow_start_time is None and ids is not None:
                    ...
                    if MARKER_ID in ids_flat:
                        ...
                        follow_done = False
                        log_event(
                            f"[INFO] Marker ID {MARKER_ID} acquired -> starting "
                            f"{FOLLOW_BEFORE_LAND_S:.1f}s follow phase"
                        )
                ...
                if SHOW_DEBUG_WINDOW:
                    ...
                    cv2.putText(
                        frame, f"MARKER LOST {lost_time:.2f}s", (20, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2
                    )

                    if follow_start_time is None:
                        cv2.putText(
                            frame, f"waiting for ID {MARKER_ID} to start follow timer", (20, 65),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2
                        )
                    else:
                        cv2.putText(
                            frame, f"follow={follow_elapsed:.1f}/{FOLLOW_BEFORE_LAND_S:.1f}s", (20, 65),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2
                        )

            if SHOW_DEBUG_WINDOW:
                cv2.imshow("Challenge1_ZED_X_Aruco_Moving", frame)
        ...
        if bridge is not None:
            try:
                bridge.send_command(0, v2v_bridge.CMD_STOP, 0)
                time.sleep(0.2)
            except Exception:
                pass
            try:
