...
# V2V BRIDGE LIBRARY (Updated for Visual Servoing)
# ========================================================
# Matches ESP32 radio binary grammar perfectly.
# Added safe helper wrappers for parametric commands.
...
class V2VBridge: # the main bridge class i wrote
    ...
    def clear_buffers(self):
        """Flushes old messages/commands before starting a new critical phase."""
        with self._lock:
    ...
    def send_command(self, cmdSeq, cmd, estop): # the sender i made for mission orders
        ...
        payload = struct.pack(CMD_FMT, int(cmdSeq), int(cmd), int(estop)) # pack it down to tiny binary
    ...
    # ─── PARAMETRIC / VISUAL SERVO HELPERS ───────────────
    def send_dynamic_move(self, distance_meters: float):
        """Drives UGV forward by a specific distance (converted to cm)"""
    ...
    def send_dynamic_turn(self, angle_degrees: float):
        """Turns UGV by specific degrees. Positive = Right/CW, Negative = Left/CCW"""
        angle = int(abs(angle_degrees))
        if angle_degrees > 0:
            self.send_command(cmdSeq=angle, cmd=CMD_TURN_ANGLE_RIGHT, estop=0)
        else:
            self.send_command(cmdSeq=angle, cmd=CMD_TURN_ANGLE_LEFT, estop=0)
            
    def send_estop(self):
        """Immediately commands the hardware to kill movement."""
