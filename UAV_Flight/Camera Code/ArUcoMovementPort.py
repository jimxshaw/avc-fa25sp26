...
# To run this script without the GUI pop-up (ignoring the fake DisplayPort), 
# append the --headless flag to your command:
#
# python3 ArUcoMovement.py --use-zed --ugv-marker-id 5 --dest-marker-id 0 \
#   --calibration ../calibration_chessboard.yaml --bridge-port /dev/ttyUSB0 \
#   --stop-distance-m 0.05 --drive-speed-mps 0.5 --headless
#
# Since there is no window to press 'q', use Ctrl+C in your terminal to stop it.
# ==============================================================================
...
def parse_args():
    ...
    parser.add_argument(
        "--headless", 
        action="store_true", 
        help="Run without opening the OpenCV video window."
    )
...
def main():
    ...
    print("Press q to quit (if window is active) or Ctrl+C in terminal.")
    print("Press s to save a screenshot (if window is active).")
    log_event(
    ...
    try:
        while True:
            ...
            if not args.headless:
                ...
                # Prevent the headless loop from hogging CPU if camera frame reads are instantaneous
                time.sleep(0.01)

    except KeyboardInterrupt:
        log_event("Interrupted by user (Ctrl+C). Shutting down...")
    finally:
        commander.close()
        cam.close()
